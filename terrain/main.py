...
DEBUG = False
reloadEx = False 
DRAW_WALLS = True  # todo: draw wall forces

SCULPT_Z_ONLY = False  # False extends the design space to organisms that can glide over debris
...
CILIA_FRAC_AFTER_FIRST_ROUND = 4  # 0.008 vs 0.002 in first round  ## this appears to have no effect!
WALL_FORCE = 10

BODY_SIZE = (7, 7, 5)

SWARM_SIZE = 9 # int(sys.argv[1]) # this should be a perfect square: 4, 9, 16, 25, etc.
...
SEED = SWARM_SIZE*1000 + EVAL_PERIOD*100 + int(sys.argv[1]) 
random.seed(SEED)
np.random.seed(SEED)
...
def tracks(data):
    ...
    tracks = design.astype(np.int8)
    # print np.sum(data), np.sum(tracks)
    if np.sum(tracks) < wx*wy*0.3333:
        return np.ones_like(tracks)
...
class MyGenotype(Genotype):
    def __init__(self):
        Genotype.__init__(self, orig_size_xyz=(wx, wy, 1), world_size=WORLD_SIZE, body_size=BODY_SIZE)
        self.add_network(CPPN(output_node_names=["track"]))
        self.to_phenotype_mapping.add_map(name="track", tag=None, func=tracks, output_type=int)
