...
def evaluate_population(pop, record_history=False):
    ...
    for n, ind in enumerate(pop[:N]):
        ...
        # new designs are evaluated with teammates from the entire population (new and old).
            ...
            (bx, by, bz) = ind.genotype.body_size
            ...
            if record_history:
                ...
                etree.SubElement(history, "RecordLink").text = '0' if pop.wall_force > 0 else '1'
            ...
            if "Data" in ind.genotype.to_phenotype_mapping:
                ...
                bodies = [body,] * pop.swarm_size  # bunch of spheres
                
                for name, details in ind.genotype.to_phenotype_mapping.items():
                    if "track" in name:
                        track_height = details["state"]
                ...
                def empty(i, j, k, track=False):
                    if track:
                        if ((world[k, i*wx+j] == 0) 
                        and (world[k-1, i*wx+j] in [0, pop.debris_mat]) and (world[k+1, i*wx+j] in [0, pop.debris_mat])
                        and (world[k, (i+1)*wx+j] in [0, 3]) and (world[k, (i-1)*wx+j] in [0, 3]) 
                        and (world[k, i*wx+j+1] in [0, 3]) and (world[k, i*wx+j-1] in [0, 3]) ):
                            return True
                ...
                bump = 0
                for i in range(1, wx-1):
                    for j in range(1, wy-1):
                        if empty(i, j, 1, 1):
                            if track_height[i,j] > 0:
